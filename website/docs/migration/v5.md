...
Use the exported helpers instead of the old method calls: `isWalkable(grid, x, y)`, `setBlocked(grid, x, y, blocked)`, `isInside(grid, x, y)`, `cloneFlatGrid(grid)`. The built-in `pathfindingAStarDiagonal`, `pathfindingAStarNoDiagonal`, and `pathfindingJumpPointNoDiagonal` already target the new contract; only a fully custom `generatePath` needs updating.
...
Presets, `createSmartEdge`, `smartEdgePresets`, `getSmartEdge`'s core params (`sourceX`/`sourceY`/.../`nodes`/`options`), floating edges, editable edges, checkpoints, hops, and `avoidAreas` all keep their v4 shape. If your app only used preset edge components without the 4.13 batch API, the only change you need is adding `SmartEdgeProvider` and controlling `nodes`.
