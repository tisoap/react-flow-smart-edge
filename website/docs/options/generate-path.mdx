...
The `grid` parameter is a `FlatGrid`: a flat typed-array obstacle grid where `blocked` is a `Uint8Array` indexed as `y * width + x`, with `1` = blocked and `0` = walkable. Helper functions are exported from the package: `createFlatGrid`, `cloneFlatGrid`, `isWalkable`, `setBlocked`, and `blockCellRange`. See the [v5 migration guide](../migration/v5) for details on upgrading custom `generatePath` implementations from v4.
