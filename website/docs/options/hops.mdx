...
that share the same `type` are bridged, so their geometry matches.

Hops use each edge's actual path: the A\* polyline when it was routed, or the
native step skeleton when `routeOnlyWhenBlocked` left the corridor clear.
A crossing still gets a bridge in that second case. You do not need a nearby
obstacle to "force" a hop.
