...
| `dragFallbackStyle`    | `{}`        | Extra style merged onto the native path while the route is pending (static dash) or an endpoint is dragging (animated wrapper). The default is `{}`.                                             |
