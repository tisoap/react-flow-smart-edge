...
| All checks (lint/type/spell)                   | `npm run check`                                                 |
| Full quality gate (check, coverage, build)     | `npm run verify`                                                |
...
Before opening a PR, run `npm run verify`.
...
- CI (`.github/workflows/ci.yml`): Node 26.3.1 → `npm ci` → `install-chromium` → `npm run verify`.
